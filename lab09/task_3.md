...
Нам дали на вход название склада. А есть ли этот склад в таблице `Warehouse`? Вдруг пользователь ввел какое-то несуществующее название или же просто ошибся в имени склада. Для этого нужно выполнить проверку. Самый очевидный способ - попробовать достать id склада по переданному нам названию и посмотреть, является он null или нет. Если такой склад реально есть, то id будет иметь какое-то не-null значение (типа `c3`). Если же такого поставщика в таблице нет, то id будет просто null. 
