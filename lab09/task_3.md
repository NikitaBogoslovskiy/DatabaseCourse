...
Нам дали на вход название склада. А есть ли этот склад в таблице `Warehouse`? Вдруг пользователь ввел какое-то несуществующее название. Для этого нужно выполнить проверку. Самый очевидный способ - попробовать достать id склада по переданному нам названию и посмотреть, является он null или нет. Если такой склад реально есть, то id будет иметь какое-то не-null значение (типа `c3`). Если же такого склада в таблице нет, то id будет просто null. 
...
```
Как будем обрабатывать такую ситуацию? Во-первых, надо бросить исключение, чтобы автор неправильного имени поставщика увидел явно, что он не прав. Во-вторых, в таблицу `log_file` надо занести какое-то описание возникшего исключения, чтобы пользователь понял, почему он не прав. Кажется, что мы можем написать такой код:
```sql
if (:warehouse_id is null) then
begin
...
```
2. Сгруппируем оставшиеся операции по товарам. То есть, например, получим группу операций по товару "Папки", группу операций по товару "Карандаши" и т.д.
```sql
select *
from operation o
where o.id_wh = :warehouse_id and o.op_date between :start_date and :end_date
group by o.id_goods
```
3. Наконец-то обратимся к select и укажем атрибуты, которые мы хотели видеть в результирующей таблице - id товара, название товара, сколько суммарно этого товара привезли за все операции, сколько этого товара суммарно увезли. Все, кроме названия товара, есть в таблице `Operation`, название же хранится в таблице `Goods`. Выполним с ней соединение:
```sql
select *
...
```
5. Ну, и осталось вывести оборот товара. Сейчас мы имеем для каждого товара группу операций, связанных с этим товаром - там могут быть как операции привоза, так и операции увоза. Как посчитать, например, только операции типа A, то есть привоза? Попробуем сделать следующее:
```sql
...
group by g.id_goods, g.nomenclature
```
Как вывести таблицу из функции, то есть записать ее в выходные переменные `goods_id`, `goods_name`, `a_quantity` и `r_quantity`? Для этого достаточно поочередно брать строки итоговой таблицы (то есть четверки с id, именем, приходом и уходом) с помощью цикла for и также поочередно их записывать в выходные параметры. В общем виде такой цикл имеет вид:
```sql
...
do suspend;
```
То есть мы берем каждую строчку таблицы, возвращаемой SQL-запросом, записываем ее в выходные параметры и с помощью команды `suspend` говорим, чтобы эти параметры забрали. Каждую итерацию эти параметры получают новые значения и каждую итерацию эти значения передаются во внешний контекст, откуда была вызвана процедура.
...
```
То каждую итерацию значения выходных параметров будут передаваться в select выше и выводиться в результат внешнего запроса, который вызвал процедуру. Как бы построчный возврат результирующей таблицы наружу, во внешний контекст. 

Если заменить SQL-запрос на наш запрос из второго пункта и выходные параметры на те, что мы реально указали, мы получим это:
```sql
